...
<jsp:include page="../fragments/navMenu.jsp"/>
    ...
    <table>
        ...
        <c:forEach items="${users}" var="user">
            <jsp:useBean id="user" scope="page" type="com.hedeen.john.app01.model.User"/>
            <tr>
                <td><c:out value="${user.name}"/></td>
                <td><a href="mailto:${user.email}">${user.email}</a></td>
                <td>${user.role}</td>
                <td><a href="users?action=edit&userid=${user.id}"><fmt:message key="user.edit"/></a></td>
                <td><a href="users?action=delete&userid=${user.id}"><fmt:message key="user.delete"/></a></td>
            </tr>
        </c:forEach>
    </table>
</body>
